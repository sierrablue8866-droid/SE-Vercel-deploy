...
/**
 * SplitHeroViewport Component
 * Toggles cleanly between the AI Smart Filter (View A) and the Interactive Virtual Tour (View B).
 * Enforces a borderless 100dvh structure with hidden overflow to maintain optimal page speeds.
 */
    ...
    <div className="relative w-full h-dvh overflow-hidden bg-[#0A1628] select-none">
      <AnimatePresence mode="wait">
        {activeView === 'A' ? (
          ...
          >
            {/* Background Motion Overlay (Simulated via compressed CSS tokens) */}
            ...
            <div className="relative z-10 w-full flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
              <div className="bg-[#10233F]/90 backdrop-blur-md border border-[#C9A84C]/20 p-4 rounded-2xl flex items-center gap-3 shadow-xl">
                ...
                <div>
                  <p className="text-xs font-bold text-[#F7F4EE]">WhatsApp Incentive Active</p>
              ...
              <button
                ...
                className="px-8 py-4 bg-gradient-to-r from-[#C9A84C] to-[#D8BB6A] text-[#0A1628] text-xs font-mono font-bold uppercase tracking-widest rounded-xl shadow-lg transition-transform duration-300 hover:scale-102 hover:shadow-[#C9A84C]/20"
              >
                Explore Virtual Telemetry Space 🔽
              </button>
            </div>
          </motion.section>
        ) : (
          ...
          >
            {/* Interactive 360 Telemetry Frame Element */}
            <div className="w-full h-full relative">
              ...
              {/* Floating Return Trigger Control */}
              <button
                onClick={() => setActiveView('A')}
                className="absolute top-24 right-6 md:right-12 bg-[#0A1628]/90 backdrop-blur-md text-[#F7F4EE] border border-[#C9A84C]/30 px-4 py-2.5 rounded-xl text-[10px] font-mono uppercase tracking-wider shadow-2xl transition-all hover:border-[#C9A84C]"
              >
